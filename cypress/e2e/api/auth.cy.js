...
describe('API - Authentication', () => {
  ...
  it('should reject an incorrect password without returning an authorization token', { tags: ['@api', '@auth', '@negative'] }, () => {
    ...
    authService.login({
      ...
      expect(loginResponse.status, 'unauthorized status').to.eq(401);
      expect(loginResponse.body.message).to.eq('Email e/ou senha inválidos');
